...
import MacrosInfoModal from '../components/MacrosInfoModal'; // Asegúrate de importar el componente correctamente
...
const MacrosScreen = () => {
  ...
  const [macros, setMacros] = useState({ carbs: null, protein: null, fat: null });
  ...
  const calculateMacros = () => {
    ...
    const fat = (dailyCalories * 0.30) / 9; // 30% of calories from fat, 9 calories per gram
    const carbs = (dailyCalories * 0.45) / 4; // 45% of calories from carbs, 4 calories per gram
  ...
  const clearInputs = () => {
    ...
    setCalories(null);
    setMacros({ carbs: null, protein: null, fat: null });
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.title}>Calculadora de Macros Diarios</Text>
        ...
        <TouchableOpacity style={[styles.button, styles.calculateButton]} onPress={calculateMacros}>
          <Text style={styles.buttonText}>Calcular</Text>
        </TouchableOpacity>

        {calories && (
          <View style={styles.resultContainer}>
            <Text style={styles.result}>Carbohidratos: {Math.round(macros.carbs)} g</Text>
            <Text style={styles.result}>Proteínas: {Math.round(macros.protein)} g</Text>
            <Text style={styles.result}>Grasas: {Math.round(macros.fat)} g</Text>
          </View>
        )}

        <TouchableOpacity onPress={() => setInfoModalVisible(true)}>
          <Text style={styles.infoText}>¿Qué son los Macros?</Text>
        </TouchableOpacity>
        ...
        <MacrosInfoModal visible={infoModalVisible} onClose={() => setInfoModalVisible(false)} />
      </ScrollView>
    </SafeAreaView>
  );
};
